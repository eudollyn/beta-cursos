async function initLoginPage() {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      setTimeout(() => location.href = result.user.role === 'admin' ? 'admin.html' : 'plataforma.html', 350);
    } catch (err) {
...
async function initPlatformPage() {
  ...
  if (!user) return;
  document.querySelectorAll('[data-user-name]').forEach(el => el.textContent = (user.name || 'aluno').split(' ')[0]);
  const [modules, progressMap] = await Promise.all([getPublishedModules(), getProgressMap()]);
  const moduleLessons = {};
  for (const module of modules) moduleLessons[module.id] = await getModuleLessons(module.id);
  ...
  target.innerHTML = modules.map(module => {
    ...
    return `
      <article class="module-card">
        ...
        <div class="progressbar" aria-label="Progresso"><span style="width:${progress}%"></span></div>
    ...
    `;
  }).join('') || '<div class="panel"><h2>Nenhum módulo publicado.</h2><p>Assim que a administração liberar o conteúdo, ele aparecerá aqui.</p></div>';
...
async function initLessonPage() {
  ...
  const [modules, allLessons, progressMap] = await Promise.all([getPublishedModules(), getModuleLessons('', false), getProgressMap()]);
  const lesson = allLessons.find(l => l.id === lessonId);
  if (!lesson) {
    document.querySelector('#lessonArea').innerHTML = '<div class="panel"><h2>Aula não encontrada</h2><p>Volte para a plataforma e escolha uma aula disponível.</p><a class="btn" href="plataforma.html">Voltar</a></div>';
    return;
  }
  ...
  if (lesson.videoUrl) {
    videoMount.innerHTML = `<video controls playsinline controlsList="nodownload" src="${escapeHtml(lesson.videoUrl)}"></video>`;
  } else {
  ...
  const lessons = allLessons.filter(l => l.moduleId === lesson.moduleId).sort((a,b) => Number(a.order || 0) - Number(b.order || 0));
  const currentIndex = lessons.findIndex(l => l.id === lesson.id);
  const next = lessons[currentIndex + 1];
  const prev = lessons[currentIndex - 1];
  document.querySelector('#nextLesson').innerHTML = next ? `<a class="btn" href="aula.html?id=${next.id}">Próxima aula</a>` : '<a class="btn secondary" href="plataforma.html">Voltar à plataforma</a>';
  document.querySelector('#prevLesson').innerHTML = prev ? `<a class="btn secondary" href="aula.html?id=${prev.id}">Aula anterior</a>` : '<a class="btn secondary" href="plataforma.html">Todos os módulos</a>';
}

async function initAdminPage() {
  const user = await requireAuth('admin');
  if (!user) return;
  document.querySelectorAll('[data-admin-name]').forEach(el => el.textContent = (user.name || 'admin').split(' ')[0]);
  setupTabs();
  setupForms();
  await refreshAdmin();
}
...
async function refreshAdmin() {
  await Promise.all([renderAdminDashboard(), renderStudents(), renderModulesAdmin(), renderLessonsAdmin()]);
}
...
async function renderAdminDashboard() {
  ...
  target.innerHTML = `
    <div class="grid-3">
      <div class="meta-card"><strong>${studentsSnap.size}</strong><span>Alunos cadastrados</span></div>
      <div class="meta-card"><strong>${modulesSnap.size}</strong><span>Módulos criados</span></div>
      <div class="meta-card"><strong>${lessonsSnap.size}</strong><span>Aulas publicadas</span></div>
    </div>
    <div class="notice"><strong>Online:</strong> alunos, módulos, aulas e progresso ficam salvos no Firebase. Os vídeos são enviados pelo painel e armazenados no Vercel Blob.</div>
  `;
}
...
async function getStudentsWithProgress() {
  ...
  return studentsSnap.docs.map(d => {
    ...
    return { ...student, progress: lessons.length ? Math.round((done / lessons.length) * 100) : 0 };
  });
}
...
async function renderStudents() {
  ...
  target.innerHTML = students.map(student => `
    <tr>
      <td><strong>${escapeHtml(student.name)}</strong><br><span class="helper">${escapeHtml(student.phone || '-')}</span></td>
      <td>${escapeHtml(student.email)}</td>
      <td><span class="badge ${student.status === 'ativo' ? 'done' : ''}">${escapeHtml(student.status)}</span></td>
      ...
      <td class="actions">
        ...
        <button class="btn small danger" data-toggle-student="${student.id}">${student.status === 'ativo' ? 'Bloquear' : 'Ativar'}</button>
      </td>
    </tr>
  `).join('') || '<tr><td colspan="5">Nenhum aluno cadastrado.</td></tr>';
  target.querySelectorAll('[data-copy-access]').forEach(btn => btn.addEventListener('click', () => {
    copyText(`Acesso ao curso Beta\nLink: ${location.origin}/login.html\nE-mail: ${btn.dataset.email}\nSenha: enviada pela administração`);
  }));
  target.querySelectorAll('[data-toggle-student]').forEach(btn => btn.addEventListener('click', async () => {
  ...
  target.querySelectorAll('[data-reset-pass]').forEach(btn => btn.addEventListener('click', async () => {
    const pass = generatePassword();
    await api('/api/reset-password', { method:'POST', json:{ uid:btn.dataset.resetPass, password:pass } });
    await renderStudents();
    copyText(`Acesso ao curso Beta\nLink: ${location.origin}/login.html\nSenha: ${pass}`);
...
async function renderModulesAdmin() {
  ...
  if (moduleSelect) moduleSelect.innerHTML = modules.map(m => `<option value="${m.id}">${escapeHtml(m.order)} - ${escapeHtml(m.title)}</option>`).join('');
  if (!target) return;
  target.innerHTML = modules.map(module => `
    <tr>
      <td>${escapeHtml(module.order)}</td>
      <td><strong>${escapeHtml(module.title)}</strong><br><span class="helper">${escapeHtml(module.description || '')}</span></td>
      <td><span class="badge ${module.status === 'publicado' ? 'done' : ''}">${escapeHtml(module.status)}</span></td>
      <td class="actions">
  ...
  target.querySelectorAll('[data-edit-module]').forEach(btn => btn.addEventListener('click', async () => {
  ...
  target.querySelectorAll('[data-delete-module]').forEach(btn => btn.addEventListener('click', async () => {
    ...
    lessons.forEach(lesson => batch.delete(db.collection('lessons').doc(lesson.id)));
    await batch.commit();
    await refreshAdmin();
  }));
}
...
async function renderLessonsAdmin() {
  ...
  if (videoSelect) videoSelect.innerHTML = lessons.map(l => `<option value="${l.id}">${escapeHtml(l.title)}</option>`).join('');
  ...
  target.innerHTML = lessons.map(lesson => {
    ...
    return `
      <tr>
        ...
        <td><strong>${escapeHtml(lesson.title)}</strong><br><span class="helper">${escapeHtml(lesson.description || '')}</span>${lesson.videoName ? `<br><span class="helper">Vídeo: ${escapeHtml(lesson.videoName)}</span>` : ''}</td>
        <td>${escapeHtml(lesson.duration || '-')}</td>
        <td><span class="badge ${lesson.status === 'publicado' ? 'done' : ''}">${escapeHtml(lesson.status)}</span></td>
        <td class="actions">
  ...
  target.querySelectorAll('[data-edit-lesson]').forEach(btn => btn.addEventListener('click', async () => {
  ...
  target.querySelectorAll('[data-delete-lesson]').forEach(btn => btn.addEventListener('click', async () => {
    if (!confirm('Excluir esta aula?')) return;
    await db.collection('lessons').doc(btn.dataset.deleteLesson).delete();
    await refreshAdmin();
  }));
  target.querySelectorAll('[data-upload-video]').forEach(btn => btn.addEventListener('click', () => {
...
function setupForms() {
  ...
  studentForm?.addEventListener('submit', async event => {
    ...
    try {
      const result = await api('/api/create-user', { method:'POST', json:{ name:studentForm.name.value.trim(), email, phone:studentForm.phone.value.trim(), password:pass } });
      studentForm.reset();
      await renderStudents(); await renderAdminDashboard();
  ...
  moduleForm?.addEventListener('submit', async event => {
    ...
    if (moduleForm.moduleId.value) await db.collection('modules').doc(moduleForm.moduleId.value).set(payload, { merge:true });
    else await db.collection('modules').add({ ...payload, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
    moduleForm.reset(); moduleForm.moduleId.value = '';
    await refreshAdmin(); toast('Módulo salvo.');
  });
  document.querySelector('#clearModuleForm')?.addEventListener('click', () => { moduleForm.reset(); moduleForm.moduleId.value = ''; });

  const lessonForm = document.querySelector('#lessonForm');
  lessonForm?.addEventListener('submit', async event => {
    event.preventDefault();
    const old = lessonForm.lessonId.value ? (await db.collection('lessons').doc(lessonForm.lessonId.value).get()).data() || {} : {};
    const payload = {
      ...old,
    ...
    if (lessonForm.lessonId.value) await db.collection('lessons').doc(lessonForm.lessonId.value).set(payload, { merge:true });
    else await db.collection('lessons').add({ ...payload, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
    lessonForm.reset(); lessonForm.lessonId.value = '';
    await refreshAdmin(); toast('Aula salva.');
  });
  document.querySelector('#clearLessonForm')?.addEventListener('click', () => { lessonForm.reset(); lessonForm.lessonId.value = ''; });
  ...
  videoForm?.addEventListener('submit', async event => {
    ...
    if (!lessonId || !file) return toast('Selecione uma aula e um arquivo de vídeo.');
    ...
    try {
      ...
      const blob = await upload(`videos/${lessonId}/${Date.now()}-${safeName}`, file, {
        ...
        clientPayload: JSON.stringify({ idToken: token, lessonId })
      });
  ...
  document.querySelector('#exportData')?.addEventListener('click', async () => {
    const [users, modules, lessons, progress] = await Promise.all([
      db.collection('users').get(), db.collection('modules').get(), db.collection('lessons').get(), db.collection('progress').get()
    ]);
    const data = {
  ...
  passwordForm?.addEventListener('submit', async event => {
    event.preventDefault();
    if (passwordForm.newPassword.value !== passwordForm.confirmPassword.value) return toast('A confirmação da senha não confere.');
    try {
      const user = auth.currentUser;
      const credential = firebase.auth.EmailAuthProvider.credential(user.email, passwordForm.currentPassword.value);
...
function insertBetaPhraseCarousel() {
  ...
  const repeated = [...phrases, ...phrases, ...phrases].map(phrase => `<span>${escapeHtml(phrase)}</span>`).join('');
...
document.addEventListener('DOMContentLoaded', async () => {
  await initLoginPage();
  if (document.body.dataset.page === 'platform') await initPlatformPage();
  if (document.body.dataset.page === 'lesson') await initLessonPage();
  if (document.body.dataset.page === 'admin') await initAdminPage();
